function loadAssets(scene: Phaser.Scene, loaderName: string, checker: (name: string) => boolean, node: any, keys: string[]): void {
    if (typeof node === "function") {
        if (!checker(node.Name)) {
            (scene.load as any)[loaderName](node.Name, ...keys.map((key: string) => node[key]));
        } else {
...
export function loadImages(scene: Phaser.Scene, node: any): void {
    loadAssets(scene, "image", scene.textures.exists.bind(scene.textures), node, ["FileURL"]);
}
...
export function loadAtlases(scene: Phaser.Scene, node: any): void {
    loadAssets(scene, "atlas", scene.textures.exists.bind(scene.textures), node, ["TextureURL", "AtlasURL"]);
}
